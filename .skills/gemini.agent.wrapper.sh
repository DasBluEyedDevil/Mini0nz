...
# Execute gemini command with positional prompt (not -p flag which is deprecated)
OUTPUT_FILE=$(mktemp)
$CMD "$FULL_PROMPT" | tee "$OUTPUT_FILE"

EXIT_CODE=${PIPESTATUS[0]}

# Read output for memory
OUTPUT=$(cat "$OUTPUT_FILE")
# Basic JSON escaping
ESCAPED_OUTPUT=${OUTPUT//\\/\\\\} # Escape backslashes
ESCAPED_OUTPUT=${ESCAPED_OUTPUT//\"/\\\"} # Escape quotes
ESCAPED_OUTPUT=${ESCAPED_OUTPUT//$'\n'/\\n} # Escape newlines
ESCAPED_OUTPUT=${ESCAPED_OUTPUT//$'\r'/} # Remove CR
ESCAPED_OUTPUT=${ESCAPED_OUTPUT//$'\t'/\\t} # Escape tabs

# Save to shared memory
MEMORY_FILE="$(dirname "$(readlink -f "$0")")/memory.json"
TIMESTAMP=$(date -u +"%Y-%m-%dT%H:%M:%SZ")

# Write JSON (ensure directory exists if script is moved but memory.json expected relative)
echo "{ \"last_analysis\": \"$ESCAPED_OUTPUT\", \"timestamp\": \"$TIMESTAMP\" }" > "$MEMORY_FILE"
